...
export async function sendWelcomeNewsletterEmail(email: string, couponCode: string = 'SAGA10'): Promise<boolean> {
        ...
        <tr>
          <td align="center">
            <table width="100%" max-width="600" border="0" cellspacing="0" cellpadding="0" style="max-width: 600px; background-color: #FFFFFF; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.06); overflow: hidden;">
              ...
              <tr>
                <td>
                  ${getEmailHeaderHTML('Welcome to SAGA VIP Circle', 'Exclusive Member Benefits & Previews')}
                </td>
              </tr>
              ...
              <tr>
                <td style="padding: 28px 24px; text-align: center;">
                  ...
                  <p style="color: #44403C; font-size: 15px; line-height: 1.6; margin-bottom: 24px;">
                    Thank you for joining our exclusive circle! As a special welcome gesture, here is your <strong>10% INSTANT DISCOUNT</strong> coupon for your first purchase.
                  </p>

                  <!-- Coupon Code Badge -->
                  <div style="background-color: #FAF6F0; border: 2px dashed #9E6962; border-radius: 12px; padding: 20px; display: inline-block; margin-bottom: 24px;">
                    <span style="color: #78716C; font-size: 11px; text-transform: uppercase; font-weight: bold; letter-spacing: 1px;">Your VIP Promo Code</span>
                    <div style="color: #9E6962; font-family: monospace; font-size: 28px; font-weight: bold; letter-spacing: 3px; margin-top: 6px;">
                      ${couponCode}
                    </div>
                    <span style="color: #137333; font-size: 12px; font-weight: bold; display: block; margin-top: 6px;">
                      ✓ Flat 10% OFF at Checkout
                    </span>
                  </div>
  ...
  try {
    const info = await transporter.sendMail({
      from: FROM_EMAIL,
      to: email,
      subject: `✨ Welcome! Enjoy 10% OFF with Code ${couponCode} | SAGA FABRICS`,
      html: htmlContent,
    });
    console.log(`[Email Service] Welcome newsletter email sent to ${email}: ${info.messageId}`);
    return true;
  } catch (error) {
    console.error(`[Email Service] Failed to send newsletter welcome email to ${email}:`, error);
    return false;
  }
}

/**
 * 5. Send Admin Newsletter Alert Email
 */
export async function sendAdminNewsletterAlertEmail(subscriberEmail: string): Promise<boolean> {
  const transporter = createTransporter();
  ...
  try {
    ...
    console.error(`[Email Service] Failed to send admin newsletter alert:`, error);
    return false;
  }
}
